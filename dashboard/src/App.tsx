...
import type { Run, ProjectStats, RunFilter, FilterOptions, Project } from './data/DataSource';
...
export default function App() {
  ...
  const [projectEmoji, setProjectEmoji] = useState<Record<string, string>>({});
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const [filterOptions, projects] = await Promise.all([
          ds.getFilterOptions(),
          ds.listProjects(),
        ]);
        setProjectEmoji(Object.fromEntries(projects.map((p: Project) => [p.name, p.name_short])));
  ...
  return (
    ...
    >
      <h1 style={{ fontSize: '1.1rem', fontWeight: 'bold', marginBottom: '0.25rem' }}>
        {(() => {
          const title = (state.status === 'ready' && state.manifest.title) ? state.manifest.title : 'Labro Dashboard';
          const mobileTitle = title.replace('Dashboard for', ':');
          return (
            <>
              <span className="proj-desktop">{title}</span>
              <span className="proj-mobile">{mobileTitle}</span>
            </>
          );
        })()}
      </h1>
      <p style={{ color: '#666', fontSize: '0.8rem', marginBottom: '0.75rem' }}>
        <span className="proj-desktop">Autonomous agent harness for unattended project tasks. Details: </span>
        <a href="https://github.com/rssrn/labro" style={{ color: '#888' }}>github.com/rssrn/labro</a>
      </p>
      ...
      {state.status === 'ready' && (
        <>
          <p style={{ color: '#666', fontSize: '0.8rem', marginBottom: '1rem' }}>
            <span className="proj-desktop">{state.runs.length} runs · </span>
            snapshot {state.manifest.generated_at}
            <span className="proj-desktop"> · {(state.manifest.size_bytes / 1024).toFixed(1)} KB · {state.manifest.content_hash.slice(0, 16)}</span>
          </p>
