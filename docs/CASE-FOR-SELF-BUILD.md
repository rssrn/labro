...
| Tool | Licence | Self-hosted? | Description | Gap vs. Labro |
|---|---|---|---|---|
| [Claude Code Routines](https://claude.com/blog/introducing-routines-in-claude-code) | Commercial (Anthropic) | ❌ Cloud only | Anthropic's native scheduling layer for Claude Code (shipped June 2026). Configure a prompt, repo, and connectors; run on cron schedules, API triggers (unique endpoint + auth token per routine), or GitHub webhooks (one session per PR, continuous updates on comments/CI failures). Draws from subscription usage limits (Pro: 5/day, Max: 15/day, Team/Enterprise: 25/day). Documented use cases: backlog triage, docs maintenance, deploy verification, alert triage, PR porting, automated code review. | No self-hosted option; requires paid Anthropic subscription; no configurable multi-source priority selection (a single prompt per routine, not a priority stack across sources); no per-project permission envelopes; no passive outcome tracking attributed to run IDs; no alert monitoring-to-GitHub pipeline; daily routine count caps; GitHub-only for webhooks (no generic webhook source). **Most direct competitor on scheduling concept**, but Claude-only and cloud-managed — Labro retains multi-provider (Codex, OpenCode, free models), self-hosted, alert monitoring integration, priority-stack, and outcome tracking as differentiators. Gaps narrow if Anthropic adds a priority-list model and outcome tracking. |
...
### Cluster 6 — Cloud-Managed Async Coding Agents (platform-native, post-2025)

This cluster did not exist at the time of the original analysis. All four tools run agents on managed cloud infrastructure and produce GitHub PRs without a human present. They are the category most likely to close the gap on Labro's scheduling and automation differentiators over time.

| Tool | Licence | Self-hosted? | Description | Gap vs. Labro |
|---|---|---|---|---|
| [Google Jules](https://jules.google/) | Commercial (Google Labs) | ❌ Cloud only | Async coding agent powered by Gemini 3.1 Pro. Tasks run on temporary Google Cloud VMs; results surface as GitHub PRs. Trigger surfaces: `jules` GitHub label, Jules CLI (`@google/jules` npm), Jules API (alpha, `AUTO_CREATE_PR` mode), and a `jules-invoke` GitHub Action supporting `schedule:` (cron), `issues:`, `pull_request:`, `push:`, and `workflow_dispatch:` events. Free tier: 15 tasks/day; Pro ($19.99/mo): ~75/day; Ultra ($124.99/mo): 300/day. SWE-bench: 51.8%. | GitHub-only VCS; no multi-source priority selection; no per-project permission envelopes; no passive outcome tracking; no alert monitoring integration; Gemini-locked (no provider choice); Jules API still alpha; per-user task caps (not team-pooled). |
| [OpenAI Codex](https://openai.com/codex/) | Commercial (OpenAI) | ❌ Cloud only | Cloud async coding agent running in network-disabled sandboxes on OpenAI infrastructure. Triggered via ChatGPT web UI, `@codex` GitHub mention, or IDE extension. Scheduled and webhook triggers are explicitly a roadmap item, not yet shipped as of mid-2026. Plus ($20/mo), Pro ($200/mo), Business ($20/user/mo), API PAYG (~$0.25/M input). | No scheduled or event-based triggers (roadmap only); `@codex` mention is the only semi-automated surface; network-off sandbox limits what agents can do during execution; OpenAI-locked; no priority stack, no permission envelopes, no outcome tracking. |
| [Cursor Cloud Agents](https://cursor.com/) | Commercial (Cursor) | ❌ Cloud only | Async coding agents running in isolated Ubuntu VMs with "computer use" capabilities: the agent launches the app, navigates the UI, records video, captures screenshots to verify its own work. Triggered from the Cursor IDE, Slack, GitHub, Linear, or mobile — all user-initiated. No cron/schedule or standalone webhook endpoint. Pro ($20/mo), Pro+ ($60/mo), Ultra ($200/mo). Up to 10–20 parallel agents. | No cron/schedule trigger; all triggering is user-initiated; Cursor-proprietary ecosystem; no priority stack, no permission envelopes, no outcome tracking; reported $40–50/mo overages on Pro at heavy usage. |
| [GitHub Copilot Coding Agent](https://github.blog/news-insights/product-news/github-copilot-meet-the-new-coding-agent/) | Commercial (GitHub/Microsoft) | ❌ Cloud only | Issue-to-PR agent running on GitHub Actions ephemeral VMs. Trigger surfaces: manual assignment (github.com, mobile, CLI, Slack, Teams, Jira, Linear, Azure Boards), `@copilot` PR mentions, and — as of June 2, 2026 — **scheduled automations** (hourly/daily/weekly) and **event automations** (new issue, PR create, PR update) with per-automation prompt + tool allowlist. Security scanning (secret protection, supply chain) runs on agent-generated code. Pro ($10/mo), Pro+ ($39/mo), Business ($19/user/mo). | Hard 59-minute session limit per task; GitHub-only VCS; scheduled/event automations currently limited to private/internal repos; single repo per task; no multi-source priority selection (one prompt per automation, not a priority stack across sources); no per-project permission envelopes; no passive outcome tracking; cannot approve its own PRs. **Highest near-term risk to Labro's scheduling differentiation** — it already ships the scheduling primitive at the lowest price point of any managed tool. |
...
|---|---|---|---|
| **GitHub Copilot Coding Agent** lifts the 59-minute session limit and extends scheduled/event automations to public repos with a priority-list model | Medium — scheduling shipped June 2, 2026; longer sessions and richer automation config are natural next steps | High — lowest entry price ($10/mo) of any managed tool, deepest GitHub integration, already ships the scheduling primitive; a priority-stack model would eliminate Labro's main scheduling advantage for users comfortable with cloud | GitHub Copilot changelog; docs.github.com/copilot/agents |
| **Claude Code Routines** adds a priority-list model (multi-source evaluation) and per-project permission envelopes | Medium — Anthropic is actively investing in Routines and the scheduling primitive already exists | Medium — cloud-managed with no self-hosting complexity, but still Claude-only (Labro retains multi-provider / free-model-via-OpenCode differentiation, plus alert monitoring integration and outcome tracking) | Anthropic blog posts; claude.ai/code changelog; `/schedule` CLI feature flags |
| **Claude Code Routines** adds outcome tracking / run observability | Medium — natural next step once scheduling is stable | Medium — narrows the compounding-value advantage; Labro retains self-hosted, multi-provider, alert monitoring integration, and priority-stack differentiation | Same as above |
