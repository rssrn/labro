...
* A *real-time* or control web UI — Labro is operated via config, CLI, and the daily digest; no UI ever triggers, pauses, or mutates runs. (A **read-only** static metrics dashboard that visualises a published copy of `labro.db` is planned post-v1 as M9 / REQ-24 — it is decoupled from the harness and changes nothing about this non-goal.)
...
2. `gh-label` — issues/PRs labelled `ai-analysis`, `ai-dev`, or `ai-review`; OR opened by `dependabot[bot]` → Claude Code (cheaper model for Dependabot, capable model for labelled issues)
...
| `gh-label` | `comment` — Labro agents do not `approve` PRs in v1 (see note below) |
...
| **REQ-24** | As an operator, I want a static, read-only web dashboard of run history and per-project metrics so I can explore Labro's activity and spend visually without querying SQLite by hand. | P2 (post-v1) | A static SPA loads a published snapshot of `labro.db` entirely client-side (no backend). It provides a filterable runs list and per-project stats, plus interactive charts driven by a shared filter bar (timespan + project / model / task-source). The dashboard is fully decoupled from the harness — it reads a periodically published DB snapshot and can never trigger, pause, or mutate runs. Run records contain prose (task descriptions, summaries) derived from potentially private repos, so the snapshot is sensitive; M9 deliberately does **not** build access control — the docs carry a prominent warning that the operator is responsible for not exposing the bucket/URL publicly. Built-in access control (e.g. Cloudflare Access) is a future option, out of scope for M9. Delivered in two phases: (1) runs list + per-project stats; (2) trend and breakdown charts. |
